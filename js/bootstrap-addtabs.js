...
$.fn.addtabs = function (options) {
    ...
    Addtabs.options = $.extend({
        ...
        iframeHeight: '750', //固定TAB中IFRAME高度,根据需要自己修改
        method: 'init',
        ...
        if (!$("#" + id)[0]) {
            ...
            var title = $('<li>', {
                ...
                $('<a>', {
                    ...
                    'role': 'tab',
                    'data-toggle': 'tab',
                    'style':'padding:5px 15px;margin-top:4px !important'
                }).html(opts.title)
            );
            ...
            if (Addtabs.options.close) {
                title.append(
                    $('<i>',{'class':'close-tab glyphicon glyphicon-remove','style':'padding-left:0;top:50%;margin-top:-4px;position:absolute;'})
                );
            }
            //创建新TAB的内容
            var content = $('<div>', {
